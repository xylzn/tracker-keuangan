...
async function createSummaryRow(spreadsheetId, dateStr) {
  ...
  const cashEndFormula   = '=IFERROR(LOOKUP(2; 1/(INDIRECT("G$2:G"& (ROW()-1))<>""); INDIRECT("G$2:G"& (ROW()-1))); 0) + INDIRECT("B"&ROW()) - INDIRECT("C"&ROW())';
...
async function ensureCashFormulas(spreadsheetId, rowIndex) {
  ...
  await sheets.spreadsheets.values.update({
    ...
    valueInputOption: "USER_ENTERED",
    requestBody: { values: [[ '0', '=IFERROR(LOOKUP(2; 1/(INDIRECT("G$2:G"& (ROW()-1))<>""); INDIRECT("G$2:G"& (ROW()-1))); 0) + INDIRECT("B"&ROW()) - INDIRECT("C"&ROW())' ]] }
  });
}
